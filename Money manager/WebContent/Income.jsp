			...
			<div class = "panel-body">
				...
				<c:choose>
	  				<c:when test = "${sessionScope.print eq true}">
	  					...
	  					<p>
			 <c:choose>
			  <c:when test = "${sessionScope.statusin eq false}">
	  			No Items retrieved
	  		  </c:when>
			  <c:otherwise>
				<table id = "t01">
				...
				<tbody>
				<c:forEach var = "item" items = "${sessionScope.in}">
				<tr>
					<td>${item.title}</td>
					<td>${item.amount}</td>	
					<td>${item.comment}</td>	
					...
					<c:otherwise>
						...
						<h3>Transactions on date</h3>
							...
							<p>
							 <c:choose>
							  <c:when test = "${sessionScope.stat2 eq false}">
					  			No Items retrieved
					  		  </c:when>
							  <c:otherwise>
								<table id = "t01">
								...
								<tbody>
								<c:forEach var = "item" items = "${sessionScope.in1}">
								<tr>
									<td>${item.title}</td>
									<td>${item.amount}</td>	
									<td>${item.comment}</td>	
